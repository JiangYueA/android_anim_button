package com.anim.button.widget.gallery;
...
import com.anim.button.BuildConfig;
...
/**
 * Created by jiangyue on 16/6/16.
 */
public class GalleryRecyclerView extends RecyclerView {
    public static final boolean DEBUG = BuildConfig.DEBUG;

    private GalleryRecyclerViewAdapter<?> mViewPagerAdapter;
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyle) {
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.GalleryRecyclerView, defStyle,
                0);
        mFlingFactor = a.getFloat(R.styleable.GalleryRecyclerView_rvp_flingFactor, 0.15f);
        mTriggerOffset = a.getFloat(R.styleable.GalleryRecyclerView_rvp_triggerOffset, 0.25f);
        mSinglePageFling = a.getBoolean(R.styleable.GalleryRecyclerView_rvp_singlePageFling, mSinglePageFling);
        a.recycle();
    }
    ...
    public GalleryRecyclerViewAdapter getWrapperAdapter() {
    ...
    public int getCurrentPosition() {
        int curPosition = -1;
        if (getLayoutManager().canScrollHorizontally()) {
            curPosition = GalleryUtils.getCenterXChildPosition(this);
        } else {
            curPosition = GalleryUtils.getCenterYChildPosition(this);
        }
        if (curPosition < 0) {
    ...
    protected void adjustPositionX(int velocityX) {
        ...
        if (childCount > 0) {
            int curPosition = GalleryUtils.getCenterXChildPosition(this);
            ...
            if (targetPosition == curPosition
                    ...
                    || !mSinglePageFling)) {
                View centerXChild = GalleryUtils.getCenterXChild(this);
    ...
    protected void adjustPositionY(int velocityY) {
        ...
        int childCount = getChildCount();
        if (childCount > 0) {
            int curPosition = GalleryUtils.getCenterYChildPosition(this);
            ...
            if (targetPosition == curPosition
                    ...
                    && mPositionOnTouchDown == curPosition)
                    || !mSinglePageFling)) {
                View centerYChild = GalleryUtils.getCenterYChild(this);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            mPositionOnTouchDown = getLayoutManager().canScrollHorizontally()
                    ? GalleryUtils.getCenterXChildPosition(this)
                    : GalleryUtils.getCenterYChildPosition(this);
            if (DEBUG) {
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == SCROLL_STATE_DRAGGING) {
            mNeedAdjust = true;
            mCurView = getLayoutManager().canScrollHorizontally() ? GalleryUtils.getCenterXChild(this) :
                    GalleryUtils.getCenterYChild(this);
            if (mCurView != null) {
            ...
            if (mNeedAdjust) {
                int targetPosition = getLayoutManager().canScrollHorizontally() ? GalleryUtils.getCenterXChildPosition(this) :
                        GalleryUtils.getCenterYChildPosition(this);
                if (mCurView != null) {
    ...
    @NonNull
    protected GalleryRecyclerViewAdapter ensureRecyclerViewPagerAdapter(Adapter adapter) {
        return (adapter instanceof GalleryRecyclerViewAdapter)
                ? (GalleryRecyclerViewAdapter) adapter
                : new GalleryRecyclerViewAdapter(this, adapter);
